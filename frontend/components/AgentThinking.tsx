...
export function AgentThinking() {
    ...
    wsManager.on('human_intervention_required', (data) => {
        setLines(prev => [...prev, `⚠️ INTERVENTION REQUIRED: ${data.reason}`])
    })
    ...
    <div className="flex-1 bg-neutral-dark border border-primary/20 p-4 rounded-lg flex flex-col gap-3 min-h-[150px] overflow-hidden font-mono text-xs">
        ...
        <div ref={containerRef} className="flex-1 overflow-y-auto space-y-1 custom-scrollbar">
            {lines.slice(-10).map((line, i) => (
                <div key={i} className={`flex gap-3 ${line.startsWith('✓') ? 'text-green-400' : line.startsWith('✗') ? 'text-red-400' : line.startsWith('⚠️') ? 'text-yellow-400 font-bold animate-pulse' : 'text-white/40'}`}>
